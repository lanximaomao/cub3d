...
/*   Updated: 2023/07/28 00:22:53 by linlinsun        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	set_distance(t_cub *cub)
{
	if (cub->var->dis_v < cub->var->dis_h)
	{
		...
		if (cub->var->ra < 90 || (cub->var->ra > 270 && cub->var->ra < 360))
			cub->var->dir = 'W';
...
static void	check_hit(t_cub *cub, int map_side_size, float *dis)
{
	while (cub->var->dof < map_side_size)
	{
		...
		cub->var->mp = cub->var->my * cub->input->map->size_x + cub->var->mx;
		if (cub->var->mp > 0 && cub->var->mp < cub->input->map->size_x
			* cub->input->map->size_y && cub->input->map->matrix[cub->var->mp
				/ cub->input->map->size_x][cub->var->mp
			% cub->input->map->size_x] == '1')
		{
			cub->var->dof = map_side_size;
			*dis = cos(deg_to_rad(cub->var->ra)) * (cub->var->rx - cub->var->px)
				- sin(deg_to_rad(cub->var->ra)) * (cub->var->ry - cub->var->py);
		}
		else
		{
...
static void	vertical(t_cub *cub)
{
	...
	if (cos(deg_to_rad(cub->var->ra)) > 0.001)
	{
		...
		cub->var->ry = (cub->var->px - cub->var->rx) * cub->var->tan
			+ cub->var->py;
		cub->var->xo = GRID_P;
...
void	horizontal(t_cub *cub)
{
	...
	if (sin(deg_to_rad(cub->var->ra)) > 0.001)
	{
		...
		cub->var->rx = (cub->var->py - cub->var->ry) * cub->var->tan
			+ cub->var->px;
		cub->var->yo = -GRID_P;
